...
It is based on CIS Ubuntu Linux 18.04 LTS Benchmark v2.0.1 from www.cisecurity.org
...
lev && (
    ...
    if  [[ ! -f /etc/systemd/system/tmp.mount ]]; then
        ...
        update_conf /etc/systemd/system/tmp.mount 'Options=mode=1777' 'Options=mode=1777,size=2G,strictatime,nosuid,nodev,noexec'
...
NO=1.5.1;     W=1; S=1; E=; SC=;  BD='Ensure permissions on bootloader config are not overridden'
lev && (
...
NO=1.6.3;     W=1; S=1; E=; SC=;  BD='Ensure prelink is not installed'
...
NO=2.1.1;     W=1; S=1; E=; SC=N; BD='Ensure xinetd is not installed'
lev && (remove_package xinted)

NO=2.1.2;     W=1; S=1; E=; SC=N; BD='Ensure openbsd-inetd is not installed'
lev && (remove_package openbsd-inetd)

NO=2.2.1.1;   W=1; S=1; E=; SC=;  BD='Ensure time synchronization is in use'
lev && (
    case ${NT} in
...
NO=2.2.3;     W=1; S=1; E=; SC=;  BD='Ensure Avahi Server is not installed'
lev && [[ -z ${SAVAHI} ]] && (remove_package avahi-daemon)

NO=2.2.4;     W=2; S=1; E=; SC=;  BD='Ensure CUPS is not installed'
lev && [[ -z ${SCUPS} ]] && (remove_package cups)

NO=2.2.5;     W=1; S=1; E=; SC=;  BD='Ensure DHCP Server is not installed'
lev && [[ -z ${SDHCPD} ]] && (
...
NO=2.2.6;     W=1; S=1; E=; SC=;  BD='Ensure LDAP server is not installed'
lev && [[ -z ${SSLAPD} ]] && (remove_package slapd)

NO=2.2.7;     W=1; S=1; E=; SC=;  BD='Ensure NFS is not installed'
lev && [[ -z ${SNFS} ]] && (remove_package nfs-kernel-server)

NO=2.2.8;     W=1; S=1; E=; SC=;  BD='Ensure DNS Server is not installed'
lev && [[ -z ${SBIND} ]] && (remove_package bind9)

NO=2.2.9;     W=1; S=1; E=; SC=;  BD='Ensure FTP Server is not installed'
lev && [[ -z ${SVSFTPD} ]] && (remove_package vsftpd)

NO=2.2.10;    W=1; S=1; E=; SC=;  BD='Ensure HTTP server is not installed'
lev && [[ -z ${SAPACHE} ]] && (remove_package apache2)

NO=2.2.11;    W=1; S=1; E=; SC=;  BD='Ensure IMAP and POP3 server are not installed'
lev && [[ -z ${SDOVECOT} ]] && (
    remove_package dovecot-imapd
    remove_package dovecot-pop3d
)

NO=2.2.12;    W=1; S=1; E=; SC=;  BD='Ensure Samba is not installed'
lev && [[ -z ${SSAMBA} ]] && (remove_package samba)

NO=2.2.13;    W=1; S=1; E=; SC=;  BD='Ensure HTTP Proxy Server is not installed'
lev && [[ -z ${SSQUID} ]] && (remove_package squid)

NO=2.2.14;    W=1; S=1; E=; SC=;  BD='Ensure SNMP Server is not installed'
...
NO=2.2.16;    W=1; S=1; E=; SC=;  BD='Ensure rsync service is not installed'
lev && [[ -z ${SRSYNC} ]] && (remove_package rsync)

NO=2.2.17;    W=1; S=1; E=; SC=;  BD='Ensure NIS Server is not installed'
...
#NO=2.3.6;     W=1; S=1; E=; SC=;  BD='Ensure RPC are not installed'
...
NO=3.1.1;     W=1; S=1; E=; SC=;  BD='Ensure packet redirect sending is disabled'
lev && (
...
NO=3.1.2;     W=1; S=1; E=; SC=;  BD='Ensure IP forwarding is disabled'
lev && (
...
NO=3.2.1;     W=1; S=1; E=; SC=;  BD='Ensure source routed packets are not accepted' 
lev && (
...
NO=3.2.2;     W=1; S=1; E=; SC=;  BD='Ensure ICMP redirects are not accepted'
lev && (
...
NO=3.2.3;     W=1; S=1; E=; SC=;  BD='Ensure secure ICMP redirects are not accepted'
lev && (
...
NO=3.2.4;     W=1; S=1; E=; SC=;  BD='Ensure suspicious packets are logged'
lev && (
...
NO=3.2.5;     W=1; S=1; E=; SC=;  BD='Ensure broadcast ICMP requests are ignored'
lev && (
    update_conf /etc/sysctl.d/local.conf 'net.ipv4.icmp_echo_ignore_broadcasts' 'net.ipv4.icmp_echo_ignore_broadcasts = 1'
)

NO=3.2.6;     W=1; S=1; E=; SC=;  BD='Ensure bogus ICMP responses are ignored'
lev && (
...
NO=3.2.7;     W=1; S=1; E=; SC=;  BD='Ensure Reverse Path Filtering is enabled'
lev && [[ -z ${PBGP} ]] && (
...
NO=3.2.8;     W=1; S=1; E=; SC=;  BD='Ensure TCP SYN Cookies is enabled'
lev && (
    update_conf /etc/sysctl.d/local.conf 'net.ipv4.tcp_syncookies' 'net.ipv4.tcp_syncookies = 1'
)

NO=3.2.9;     W=1; S=1; E=; SC=;  BD='Ensure IPv6 router advertisements are not accepted'
lev && ip6 && (
...
NO=3.3.1;     W=1; S=1; E=; SC=;  BD='Ensure TCP Wrappers is installed'
lev  # Not needed because firewall is installed

NO=3.3.2;     W=1; S=1; E=; SC=;  BD='Ensure /etc/hosts.allow is configured'
lev # Not needed because firewall is installed

NO=3.3.3;     W=1; S=1; E=; SC=;  BD='Ensure /etc/hosts.deny is configured'
lev # Not needed because firewall is installed

NO=3.3.4;     W=1; S=1; E=; SC=;  BD='Ensure permissions on /etc/hosts.allow are configured'
lev # Not needed because firewall is installed

NO=3.3.5;     W=1; S=1; E=; SC=;  BD='Ensure permissions on /etc/hosts.deny are configured'
lev # Not needed because firewall is installed
...
NO=3.5.1.1;   W=1; S=1; E=; SC=;  BD='Ensure a firewall package is installed'
...
NO=3.5.2.1;   W=1; S=1; E=; SC=;  BD='Ensure ufw service is enabled'
lev && UFW && (
...
NO=3.5.2.2;   W=1; S=1; E=; SC=;  BD='Ensure loopback traffic is configured'
lev && UFW && (
...
NO=3.5.3.3;   W=1; S=1; E=; SC=N; BD='Ensure outbound connections are configured'
lev && UFW && (
...
NO=3.5.2.4;   W=1; S=1; E=; SC=N; BD='Ensure firewall rules exist for all open ports'
lev && UFW && (
    # tcp
    while read PORT; do
...
NO=3.5.2.5;   W=1; S=1; E=; SC=;  BD='Ensure default deny firewall policy'
lev && UFW && (
...
NO=3.5.3.1;   W=1; S=1; E=; SC=;  BD='Ensure nftables is installed'
lev && nft && (install_package nftables) 

NO=3.5.3.2;   W=1; S=1; E=; SC=;  BD='Ensure ufw is uninstalled or disabled with nftables'
lev && nft && (remove_package ufw)

NO=3.5.3.3;   W=1; S=1; E=; SC=N; BD='Ensure iptables are flushed with nftables'
lev && nft && (
    upd && iptables -F
    upd && ip6tables -F
)

NO=3.5.3.4;   W=1; S=1; E=; SC=;  BD='Ensure a nftables table exists'
lev && nft && upd && (nft create table inet filter)

NO=3.5.3.5;   W=1; S=1; E=; SC=;  BD='Ensure nftables base chains exist'
lev && nft && (
...
NO=3.5.3.6;   W=1; S=1; E=; SC=;  BD='Ensure nftables loopback traffic is configured'
lev && nft && (
...
NO=3.5.3.7;   W=1; S=1; E=; SC=N; BD='Ensure nftables outbound and established connections are configured'
lev && nft && (
...
NO=3.5.3.8;   W=1; S=1; E=; SC=;  BD='Ensure nftables default deny firewall policy'
lev && nft && (
...
NO=3.5.3.9;   W=1; S=1; E=; SC=;  BD='Ensure nftables service is enabled'
lev && nft && upd && (systemctl enable nftables)

NO=3.5.3.10;  W=1; S=1; E=; SC=;  BD='Ensure nftables rules are permanent'
lev && nft && (update_conf /etc/nftables.conf 'include "/etc/nftables.rules"')
 
NO=3.5.4.1.1; W=1; S=1; E=; SC=;  BD='Ensure iptables packages are installed'
lev && ipt && (
    install_package iptables
    install_package iptables-persistent
)

NO=3.5.4.1.2; W=1; S=1; E=; SC=;  BD='Ensure nftables is not installed with iptables'
lev && ipt && (remove_package nftables) 

NO=3.5.4.1.3; W=1; S=1; E=; SC=;  BD='Ensure ufw is uninstalled or disabled with iptables'
lev && ipt && (remove_package ufw) 

NO=3.5.4.1.4; W=1; S=1; E=; SC=;  BD='Ensure iptables loopback traffic is configured'
lev && ipt && (
...
NO=3.5.4.1.5; W=1; S=1; E=; SC=N; BD='Ensure iptables outbound and established connections are configured'
lev && ipt && (
...
NO=3.5.4.1.6; W=1; S=1; E=; SC=;  BD='Ensure iptables default deny firewall policy'
lev && ipt && (
...
NO=3.5.4.1.7; W=1; S=1; E=; SC=;  BD='Ensure iptables firewall rules exist for all open ports'
lev && ipt && (
...
NO=3.5.4.2.1; W=1; S=1; E=; SC=;  BD='Ensure ip6tables loopback traffic is configured'
lev && ipt && ip6 &&  (
...
NO=3.5.4.2.2; W=1; S=1; E=; SC=;  BD='Ensure ip6tables outbound and established connections are configured'
lev && ipt && ip6 && (
...
NO=3.5.3.2.3; W=1; S=1; E=; SC=;  BD='Ensure ip6tables default deny firewall policy'
lev && ipt && ip6 && (
...
NO=3.5.3.2.4; W=1; S=1; E=; SC=N; BD='Ensure ip6tables firewall rules exist for all open ports'
lev && ipt && ip6 && (
...
NO=3.6;       W=2; S=1; E=; SC=N; BD='Ensure wireless interfaces are disabled'
lev && (
    install_package network-manager
    upd && nmcli radio all off
)

NO=3.7;       W=2; S=2; E=; SC=N; BD='Disable IPv6'
...
NO=5.2.4;     W=1; S=1; E=; SC=;  BD='Ensure SSH access is limited'
...
NO=5.2.18;    W=1; S=1; E=; SC=;  BD='Ensure SSH warning banner is configured'
lev && ssd && (update_conf /etc/ssh/sshd_config 'Banner' 'Banner /etc/issue.net')

NO=5.2.19;    W=1; S=1; E=; SC=;  BD='Ensure SSH PAM is enabled'
lev && ssd && (update_conf /etc/ssh/sshd_config 'UsePAM' 'UsePAM yes')

NO=5.2.20;    W=1; S=1; E=; SC=;  BD='Ensure SSH AllowTcpForwarding is disabled'
lev && ssd && (update_conf /etc/ssh/sshd_config 'AllowTcpForwarding' 'AllowTcpForwarding no')

NO=5.2.21;    W=1; S=1; E=; SC=;  BD='Ensure SSH MaxStartups is configured'
lev && ssd && (update_conf /etc/ssh/sshd_config "MaxStartups" "MaxStartups ${SSHMAXST}")

NO=5.2.22;    W=1; S=1; E=; SC=;  BD='Ensure SSH MaxSessions is is limited'
