...
[[ -s ${CISRC} ]] || {
    ...
    case $? in
        0) echo -e 'TL="W1"                                            # Type of system and level S1, S2, W1, W2.  ' >> ${CISRC}
           echo -e 'SX11="Y"                                           # Set to Y to prevent deletion.             ' >> ${CISRC}
           echo -e 'SAVAHI="Y"                                         # Set to Y to prevent deletion.             ' >> ${CISRC} ;;
        *) echo -e 'TL="S1"                                            # Type of system and level S1, S2, W1, W2.  ' >> ${CISRC}
           echo -e 'SX11=""                                            # Set to Y to prevent deletion.             ' >> ${CISRC}
           echo -e 'SAVAHI=""                                          # Set to Y to prevent deletion.             ' >> ${CISRC} ;;
    esac

    apt list --installed 2> /dev/null | grep -q openssh-server
    case $? in
        0) echo -e 'SSSHD="Y"                                          # Set to Y to prevent deletion.             ' >> ${CISRC} ;;
        *) echo -e 'SSSHD=""                                           # Set to Y to prevent deletion.             ' >> ${CISRC} ;;
    esac

    echo -e 'SCUPS=""                                           # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SDHCPD=""                                          # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SSLAPD=""                                          # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SNFS=""                                            # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SBIND=""                                           # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SVSFTPD=""                                         # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SAPACHE=""                                         # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SDOVECOT=""                                        # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SSAMBA=""                                          # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SSQUID=""                                          # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SSNMPD=""                                          # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SRSYNC=""                                          # Set to Y to prevent deletion.             ' >> ${CISRC}
    echo -e 'SNIS=""                                            # Set to Y to prevent deletion.             ' >> ${CISRC}
    ...
    echo -e 'PASSWARNAGE="7"                                    # Set password expiration warning days.     ' >> ${CISRC}
...
# Parameter 1 = (maximum,minimum, warning,inactive)
...
function update_chage() {
    ...
    do 
        CHECK=$(chage -l ${USR} | grep -i "${1}" | awk -F: {'print $2'} | sed s/' '//g)
        [[ ${CHECK} = "never" ]] && CHECK="-1"
        if [[ ${1} != last ]]; then
            if [[ ${CHECK} -eq ${2} ]]; then
                prn "Password ${1} days for ${USR} is already set to ${2}."
            else
                upd || prw "Password ${1} days for ${USR} is set to ${CHECK} and needs to be changed to ${2}."
                upd && prw "Setting password ${1} days for ${USR} to ${2}."
                case ${1} in
                    maximum)  upd && chage --maxdays  ${2} ${USR} ;;
                    minimum)  upd && chage --mindays  ${2} ${USR} ;;
                    warning)  upd && chage --warndays ${2} ${USR} ;;
                    inactive) upd && chage --inactive ${2} ${USR} ;;
                esac
            fi
        else
            CHECK=$(chage -l  ${USR} | grep -i ${1} | awk -F: {'print $2'})
            CHECK=$(date -d "${CHECK}" +"%Y%m%d")
            if [[ ${CHECK} -gt ${TODAY} ]]; then
                prw "Last password change for ${USR} is in the future (${CHECK}). CHECK SYSTEM!"
            else
    ...
    upd && (
        ...
        case $? in
            ...
            *)  echo -e "\n\nSet grub password. Use at least 15 characters.\n"
                ...
                grep "^PBKDF2" -q ${TMP1}
                case $? in
                    0)  update_conf /etc/grub.d/${GRF} "set superusers=${SUDO_USER:-root}"
                        echo -e "password_pbkdf2 ${SUDO_USER:-root} $(grep "^PBKDF2" ${TMP1} | awk -F "is " '{print $2}')" >> /etc/grub.d/${GRF}
                        update_grub ;;
                esac ;;
        esac
    )
...
lev && (update_conf /etc/sysctl.conf 'kernel.randomize_va_space' 'kernel.randomize_va_space = 2')
...
lev && (
    ...
    update_conf /etc/sysctl.conf 'fs.suid_dumpable' 'fs.suid_dumpable = 0'
...
lev && (
    update_conf /etc/sysctl.conf 'net.ipv4.conf.all.send_redirects' 'net.ipv4.conf.all.send_redirects = 0'
    update_conf /etc/sysctl.conf 'net.ipv4.conf.default.send_redirects' 'net.ipv4.conf.default.send_redirects = 0'
)

NO=3.2.2;     W=1; S=1; E=; SC=;  BD='Ensure IP forwarding is disabled'
lev && (
    update_conf /etc/sysctl.conf 'net.ipv4.ip_forward' 'net.ipv4.ip_forward = 0'
    update_conf /etc/sysctl.conf 'net.ipv6.conf.all.forwarding' 'net.ipv6.conf.all.forwarding = 0' 
)

NO=3.3.1;     W=1; S=1; E=; SC=;  BD='Ensure source routed packets are not accepted'
lev && (
    update_conf /etc/sysctl.conf 'net.ipv4.conf.all.accept_source_route' 'net.ipv4.conf.all.accept_source_route = 0'
    update_conf /etc/sysctl.conf 'net.ipv4.conf.default.accept_source_route' 'net.ipv4.conf.default.accept_source_route = 0'
    update_conf /etc/sysctl.conf 'net.ipv6.conf.all.accept_source_route' 'net.ipv6.conf.all.accept_source_route = 0'
    update_conf /etc/sysctl.conf 'net.ipv6.conf.default.accept_source_route' 'net.ipv6.conf.default.accept_source_route = 0'
)

NO=3.3.2;     W=1; S=1; E=; SC=;  BD='Ensure ICMP redirects are not accepted'
lev && (
    update_conf /etc/sysctl.conf 'net.ipv4.conf.all.accept_redirects' 'net.ipv4.conf.all.accept_redirects = 0'
    update_conf /etc/sysctl.conf 'net.ipv4.conf.default.accept_redirects' 'net.ipv4.conf.default.accept_redirects = 0'
    update_conf /etc/sysctl.conf 'net.ipv6.conf.all.accept_redirects' 'net.ipv6.conf.all.accept_redirects = 0'
    update_conf /etc/sysctl.conf 'net.ipv6.conf.default.accept_redirects' 'net.ipv6.conf.default.accept_redirects = 0'
)

NO=3.3.3;     W=1; S=1; E=; SC=;  BD='Ensure secure ICMP redirects are not accepted'
lev && (
    update_conf /etc/sysctl.conf 'net.ipv4.conf.all.secure_redirects' 'net.ipv4.conf.all.secure_redirects = 0'
    update_conf /etc/sysctl.conf 'net.ipv4.conf.default.secure_redirects' 'net.ipv4.conf.default.secure_redirects = 0'
)

NO=3.3.4;     W=1; S=1; E=; SC=;  BD='Ensure suspicious packets are logged'
lev && (
    update_conf /etc/sysctl.conf 'net.ipv4.conf.all.log_martians' 'net.ipv4.conf.all.log_martians = 1'
    update_conf /etc/sysctl.conf 'net.ipv4.conf.default.log_martians' 'net.ipv4.conf.default.log_martians = 1'
)

NO=3.3.5;     W=1; S=1; E=; SC=;  BD='Ensure broadcast ICMP requests are ignored'
lev && (update_conf /etc/sysctl.conf 'net.ipv4.icmp_echo_ignore_broadcasts' 'net.ipv4.icmp_echo_ignore_broadcasts = 1')

NO=3.3.6;     W=1; S=1; E=; SC=;  BD='Ensure bogus ICMP responses are ignored'
lev && (update_conf /etc/sysctl.conf 'net.ipv4.icmp_ignore_bogus_error_responses' 'net.ipv4.icmp_ignore_bogus_error_responses = 1')

NO=3.3.7;     W=1; S=1; E=; SC=;  BD='Ensure Reverse Path Filtering is enabled'
lev && (
    update_conf /etc/sysctl.conf 'net.ipv4.conf.all.rp_filter' 'net.ipv4.conf.all.rp_filter = 1'
    update_conf /etc/sysctl.conf 'net.ipv4.conf.default.rp_filter' 'net.ipv4.conf.default.rp_filter = 1'
)

NO=3.3.8;     W=1; S=1; E=; SC=;  BD='Ensure TCP SYN Cookies is enabled'
lev && (update_conf /etc/sysctl.conf 'net.ipv4.tcp_syncookies' 'net.ipv4.tcp_syncookies = 1')

NO=3.3.9;     W=1; S=1; E=; SC=;  BD='Ensure IPv6 router advertisements are not accepted'
lev && (
    update_conf /etc/sysctl.conf 'net.ipv6.conf.all.accept_ra' 'net.ipv6.conf.all.accept_ra = 0'
    update_conf /etc/sysctl.conf 'net.ipv6.conf.default.accept_ra' 'net.ipv6.conf.default.accept_ra = 0'
)
...
lev && [[ ${FW} = ufw ]] && (
    pfw && (
        ...
        err && ufw enable
        case ${IPV6} in
            Y) update_conf /etc/default/ufw 'IPV6' 'IPV6=yes' ;;
            *) update_conf /etc/default/ufw 'IPV6' 'IPV6=no' ;;
        esac
    )
)
...
lev && [[ ${FW} = ufw ]] && (
    pfw && ufw allow in on lo
    pfw && ufw deny in from 127.0.0.0/8
    case ${IPV6} in
        Y) pfw && ufw deny in from ::1 ;;
    esac
)
...
lev && [[ ${FW} = "iptables" ]] && [[ ${IPV6} ]] && (
    pfw && ip6tables -A INPUT -p <protocol> --dport <port> -m state --state NEW -j ACCEPT
)
...
lev && (update_conf /etc/default/grub 'GRUB_CMDLINE_LINUX="audit=1"') 
...
lev && [[ ${SL} = "rsyslog" ]] && (
    update_conf /etc/rsyslog.d/50-default.conf '$FileCreateMode' '$FileCreateMode 0640'
    update_conf /etc/rsyslog.conf '$FileCreateMode' '$FileCreateMode 0640'
)
...
lev && (
    (grep -E "^\s*create\s+\S+" /etc/logrotate.conf | grep -qv "0640") && {
        upd || prw "/etc/logrotate.conf contains wrong create file permissions and needs to be updated."
        upd && prw "/etc/logrotate.conf contains wrong create file permissions. Updating."
        upd &&  sed -i 's/0[0-7][0-7][0-7]/0640/g' /etc/logrotate.conf
    }
    err  || prn "/etc/logrotate.conf create file permissions are already correct."
)
...
lev && [[ ${SSSHD} ]] && (
    ...
    update_conf /etc/ssh/sshd_config 'ClientAliveCountMax' 'ClientAliveCountMax 0'
)
...
lev && [[ ${SSSHD} ]] && [[ ${SUDO_USER} ]] && (update_conf /etc/ssh/sshd_config 'AllowUsers' "AllowUsers ${SUDO_USER}")
...
lev && [[ ${SSSHD} ]] && (update_conf /etc/ssh/sshd_config 'MaxSessions' 'MaxSessions 4')
...
lev && (
    ...
    update_chage maximum ${PASSMAXDAYS}
)
...
lev && (
    ...
    #update_conf /etc/group "root:x:0:" "root:x:0:${SUDO_USER}"
)
...
lev && (
    ...
    for DIR in $(echo $PATH | tr ":" " "); do
        ...
        if [[ -d ${DIR} ]]; then
            (ls -ldH ${DIR} | cut -c6 | grep -qv "-")          && prw "Group Write permission set on directory ${DIR}"
            (ls -ldH ${DIR} | cut -c9 | grep -qv "-")          && prw "Other Write permission set on directory ${DIR}"
            [[ $(ls -ldH ${DIR} | awk '{print $3}') != "root" ]] && prw "Directory ${DIR} is not owned by root."
        else
...
lev && (
    ...
    update_conf /etc/sysctl.conf 'net.core.default_qdisc=fq'
    update_conf /etc/sysctl.conf 'net.ipv4.tcp_congestion_control=bbr'
)
