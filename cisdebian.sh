...
VERSION=20220612
################################### HARDENING SCRIPT FOR DEBIAN 11 ########################### 
...
The purpose of this script is to harden Debian Linux systems.
It is based on CIS Debian Family Linux Benchmark v1.1.0 (Draft) from www.cisecurity.org
https://www.cisecurity.org/cis-securesuite/cis-securesuite-membership-terms-of-use/
It has only been tested on Debian 11 X64.
...
[[ -s ${CISRC} ]] || {
    ...
    apt list --installed 2> /dev/null | grep -q debian-desktop
    case $? in
...
function update_modprobe() {
    ...
    case $? in
        ...
        *)  upd || prw "File /etc/modprobe.d/${1}.conf needs to be created."
            upd && prw "Updating /etc/modprobe.d/${1}.conf with install ${1} /bin/false."
            upd && echo "install ${1} /bin/false" > /etc/modprobe.d/${1}.conf ;;
    esac
}
...
NO=1.2.2;     W=1; S=1; E=; SC=;  BD='Ensure /tmp is configured'
lev && (
    check_fstab /tmp
...
NO=1.2.3;     W=1; S=1; E=; SC=;  BD='Ensure nodev option set on /tmp partition'
#change to systemd tmpfs
lev && (update_fstab /tmp 'defaults,nodev,nosuid,noexec')

NO=1.2.4;     W=1; S=1; E=; SC=;  BD='Ensure nosuid option set on /tmp partition'
lev  # Updated in 1.1.3

NO=1.1.5;     W=1; S=1; E=; SC=;  BD='Ensure noexec option set on /tmp partition'
lev  # Updated in 1.1.3

NO=1.2.6;     W=1; S=1; E=; SC=;  BD='Ensure /dev/shm is configured'
lev && (
...
NO=1.2.7;     W=1; S=1; E=; SC=;  BD='Ensure nodev option set on /dev/shm partition'
lev  # Updated in 1.1.6

NO=1.2.8;     W=1; S=1; E=; SC=;  BD='Ensure nosuid option set on /dev/shm partition'
lev  # Updated in 1.1.6

NO=1.2.9;     W=1; S=1; E=; SC=;  BD='Ensure noexec option set on /dev/shm partition'
lev  # Updated in 1.1.6

NO=1.2.10;    W=2; S=2; E=; SC=;  BD='Ensure separate partition exists for /var'
lev && (check_fstab /var)

NO=1.2.11;    W=2; S=2; E=; SC=;  BD='Ensure separate partition exists for /var/tmp'
lev && (check_fstab /var/tmp)

NO=1.2.12;    W=2; S=2; E=; SC=;  BD='Ensure nodev option set on /var/tmp partition'
lev && (update_fstab /var/tmp 'defaults,nodev,nosuid,noexec')

NO=1.2.13;    W=2; S=2; E=; SC=;  BD='Ensure nosuid option set on /var/tmp partition'
lev  # Updated in 1.1.12

NO=1.2.14;    W=2; S=2; E=; SC=;  BD='Ensure noexec option set on /var/tmp partition'
lev  # Updated in 1.1.12

NO=1.2.15;    W=2; S=2; E=; SC=;  BD='Ensure separate partition exists for /var/log'
lev && (check_fstab /var/log) 

NO=1.2.16;    W=2; S=2; E=; SC=;  BD='Ensure separate partition exists for /var/log/audit'
lev && (check_fstab /var/log/audit)

NO=1.2.17;    W=2; S=2; E=; SC=;  BD='Ensure separate partition exists for /home'
lev && (check_fstab /home)
 
NO=1.2.18;    W=1; S=1; E=; SC=;  BD='Ensure nodev option set on /home partition'
lev && (update_fstab /home 'defaults,nodev')

NO=1.2.19;    W=1; S=1; E=; SC=N; BD='Ensure nodev option set on removable media partitions'
lev && (update_fstab /dev/cdrom 'defaults,nodev,nosuid,noexec')

NO=1.2.20;    W=1; S=1; E=; SC=N; BD='Ensure nosuid option set on removable media partitions'
lev  # Updated in 1.1.19

NO=1.2.21;    W=1; S=1; E=; SC=N; BD='Ensure noexec option set on removable media partitions'
lev  # Updated in 1.1.19

NO=1.2.22;    W=1; S=1; E=; SC=N; BD='Ensure sticky bit is set on all world-writable directories'
lev && (
...
NO=1.2.23;    W=2; S=1; E=; SC=;  BD='Disable Automounting'
lev && (remove_package autofs)

NO=1.2.24;    W=2; S=1; E=; SC=;  BD='Disable USB Storage'
...
NO=1.3.1;     W=1; S=1; E=; SC=N; BD='Ensure package manager repositories are configured'
lev && (
    apt-cache policy | grep http | grep -vq "debian.org"
    (($? != 1)) && prw "Found repositories that are not Ubuntu repositories. Check apt policy."
    err         || prn "Only Ubuntu repositories installed"
)

NO=1.3.2;     W=1; S=1; E=; SC=N; BD='Ensure GPG keys are configured'
lev && (
...
NO=1.5.3;     W=1; S=1; E=; SC=;  BD='Ensure permissions on bootloader config are configured'
lev && (update_grub)

NO=1.5.4;     W=1; S=1; E=; SC=;  BD='Ensure authentication required for single user mode'
lev && (
...
NO=1.8.1;     W=1; S=1; E=; SC=;  BD='Ensure message of the day is configured properly'
lev && (update_message /etc/motd)

NO=1.8.2;     W=1; S=1; E=; SC=;  BD='Ensure local login warning banner is configured properly'
lev && (update_message /etc/issue)

NO=1.8.3;     W=1; S=1; E=; SC=;  BD='Ensure remote login warning banner is configured properly'
lev && (update_message /etc/issue.net)

NO=1.8.4;     W=1; S=1; E=; SC=;  BD='Ensure permissions on /etc/motd are configured'
lev && (update_file /etc/motd root root 644)

NO=1.8.5;     W=1; S=1; E=; SC=;  BD='Ensure permissions on /etc/issue are configured'
lev && (update_file /etc/issue root root 644)

NO=1.8.6;     W=1; S=1; E=; SC=;  BD='Ensure permissions on /etc/issue.net are configured'
lev && (update_file /etc/issue.net root root 644)

NO=1.9.1;     W=3; S=2; E=; SC=N; BD='Ensure GNOME Display Manager is removed'
lev && (remove_package gdm3)

NO=1.9.2;     W=1; S=1; E=; SC=;  BD='Ensure GDM login banner is configured'
lev && (
...
NO=1.9.3;     W=3; S=1; E=; SC=;  BD='Ensure disable-user-list is configured'
lev && (
    if [[ -s /etc/gdm3/greeter.dconf-defaults ]]; then
...
NO=1.9.4;     W=1; S=1; E=; SC=;  BD='Ensure XDCMP is not enabled'
lev && (
...
NO=1.10;      W=1; S=1; E=; SC=N; BD='Ensure updates, patches, and additional security software are installed'
lev && (
...
NO=4.2.3;     W=1; S=1; E=; SC=N; BD='Ensure logrotate is configured'
lev && (update_conf /etc/logrotate.conf "maxage" "maxage ${MAXLOGAGE}")

NO=4.2.4;     W=1; S=1; E=; SC=;  BD='Ensure logrotate assigns appropriate permissions'
lev && (update_conf /etc/logrotate.conf 'create' 'create 0640 root utmp')

NO=4.2.5;     W=1; S=1; E=; SC=;  BD='Ensure permissions on all logfiles are configured'
lev && (
...
NO=5.3.13;    W=1; S=1; E=; SC=;  BD='Ensure only strong MAC algorithms are used'
...
NO=5.3.14;    W=1; S=1; E=; SC=;  BD='Ensure only strong Key Exchange algorithms are used'
...
NO=5.3.15;    W=1; S=1; E=; SC=;  BD='Ensure SSH Idle Timeout Interval is configured'
lev && ssd && (
...
NO=5.3.16;    W=1; S=1; E=; SC=;  BD='Ensure SSH LoginGraceTime is set to one minute or less'
lev && ssd && (update_conf /etc/ssh/sshd_config 'LoginGraceTime' 'LoginGraceTime 60')

NO=5.3.17;    W=1; S=1; E=; SC=;  BD='Ensure SSH access is limited'
...
NO=5.4.2;     W=3; S=3; E=; SC=;  BD='Ensure lockout for failed password attempts is configured'
## Debian 11 uses faillock. Enabling this benchmark will disable login 
lev && (
